...
class HighFiveController extends ControllerBase {
  ...
  public function about() {
    return array(
      '#markup' => '<p>' . $this->t('A gesture of celebration or greeting in which two people slap each others open palm with their arms raised.') . '</p>',
    );
  }
  ...
  public function performHighFive($first, $second) {
    ...
    $list[] = $this->t("First person was @number.", array('@number' => $first));
    $list[] = $this->t("Second person was @number.", array('@number' => $second));
    $list[] = $this->t('@person1 and @person2 gave each other a high five.', array('@person1' => $first, '@person2' => $second));

    $render_array['HighFive_arguments'] = array(
      // The theme function to apply to the #items.
      '#theme' => 'item_list',
      // The list itself.
      '#items' => $list,
      '#title' => $this->t('High Five Status'),
    );
    return $render_array;
  }
